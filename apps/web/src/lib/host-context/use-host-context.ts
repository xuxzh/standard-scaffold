 ...
 * micro host mode this is the most recent snapshot pushed by the host.
 *
 * The underlying store only changes identity when the host actually pushes
 * through qiankun props, so this hook re-renders the component only on real
 * updates (login, language switch, etc.).
 */
