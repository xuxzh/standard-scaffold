...
在当前 `main` 分支、`709889e` 基线上实施方案 B。当前 Web 测试为 6 个文件、18 个用例失败，并产生 7 个未处理异步错误；修复仅覆盖这些阻断项，不处理现有 11 条 lint warning，不修改当前产品行为。
...
- 物料包装关系列表失败测试只验证页面不渲染持久错误条；删除失败测试验证弹窗保持打开且不触发成功流程。直接 mock query/mutation 的组件测试不再断言全局 QueryCache 通知。
...
- 成功后的关闭弹窗、刷新状态等逻辑仍只在 mutation 成功后执行；失败时表单或确认弹窗保持可操作。
...
- mutation 失败后不执行成功分支，相关表单或确认弹窗仍保持可操作。
- 列表请求失败时不出现持久内联错误条。
...
- 按用户授权直接在当前 `main` 分支修改，不创建任务分支或 worktree。
