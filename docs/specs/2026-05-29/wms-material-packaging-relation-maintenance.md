...
- `docs/business/mes/packaging/material-packaging-relation/assets/03-编辑物料关系.png`
...
编辑态保留物料编码和包装规则编码的选择入口。重新选择物料时同步更新 `MaterialCode` 与 `MaterialName`；重新选择包装规则时同步更新 `PackagingRuleCode` 与 `PackagingRuleName`，并以新规则明细替换当前弹窗内的完整明细列表。若后端因物料编码 + 包装规则编码组合重复返回错误，页面保留当前弹窗输入并展示错误消息。
...
- `Remark`：可选，长度 `0-200`。
- `Details`：至少 `1` 条。选择包装规则后默认生成明细；重新选择包装规则时以新规则明细替换旧明细。用户可以调整数量和模板字段。
...
- 明细表展示层级、规格、单位、包装类型等只读字段，并允许用户调整包装数量、箱标签打印模板和装箱单打印模板。
- 点击“重置”后清空本次新增弹窗的当前输入，恢复到初始空表单。
- 提交成功后关闭弹窗，刷新当前列表。
- 提交失败时保留输入并展示后端错误消息。
...
- `MaterialCode` 和 `PackagingRuleCode` 仍使用选择输入；`MaterialName` 和 `PackagingRuleName` 只读展示，由选择结果带出。
- 重新选择物料时同步更新物料编码和物料名称。
- 重新选择包装规则时同步更新包装规则编码、规则名称，并以新规则明细替换当前明细列表。
- 用户可修改备注、包装数量、箱标签打印模板和装箱单打印模板；层级、规格、单位和包装类型字段只读展示。
- 点击“重置”后恢复为本次打开编辑弹窗时加载的原始值，而不是清空表单。
...
- 新增或编辑成功后不强制失效物料候选和包装规则候选，因为关系维护不会改变物料主数据或包装规则候选数据。
...
- page 测试：验证 loading、empty、error、左侧物料筛选、右侧筛选提交、列表展平渲染、新增、编辑态重选物料和包装规则、编辑重置恢复打开时原值、物料选择、包装规则选择、明细数量校验、删除和批删。
