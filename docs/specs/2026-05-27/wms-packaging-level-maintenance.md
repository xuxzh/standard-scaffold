...
包装层级维护属于 WMS 基础数据中心，用于维护包装主数据之间的层级嵌套关系。层级序号从 1 开始，序号越大表示越外层；父层层级只能选择序号小于当前层级的层级。
...
新增/编辑表单和筛选区的“父层层级编码”需要候选项。首版通过同一个查询接口获取候选项：
...
- 当输入的层级序号为 `1` 时，内层下拉禁用并清空。
- 当输入的层级序号大于 `1` 时，内层下拉只展示序号小于当前层级序号的候选项。
- 选择内层后，父级层级名称只读展示。
...
- 选项查询失败：内层下拉可为空，提交时依赖后端校验，并展示错误提示。
...
- page 测试：验证 loading、empty、error、列表渲染、筛选提交、内层候选过滤、表单提交、删除、批量删除、关系图弹窗。
